...
fn main() {
    ...
    for (i, experience) in experiences.iter().enumerate() {
        ...
        println!("  → Created semantic event with {} atoms, {} relationships", 
                 event.atoms.len(), event.relationships.len());
        ...
        for fragment in &fragments {
            match &fragment.content {
                FragmentContent::CausalRule { condition, outcome, confidence } => {
                    println!("    • Causal Rule: {} → {} (conf: {:.2})", 
                             condition, outcome, confidence);
                }
                FragmentContent::EntityRelation { entity, relation, target } => {
                    println!("    • Entity Relation: {} {} {} (conf: {:.2})", 
                             entity, relation, target, fragment.confidence);
                }
                FragmentContent::GoalStrategy { goal, strategy, success_rate } => {
                    println!("    • Goal Strategy: {} → {} (success: {:.2})", 
    ...
    for (goal, domain) in &goals {
        ...
        println!("    - Estimated execution time: {:.2}s", eeg.metadata.estimated_execution_time);
        ...
        for (id, node) in &eeg.nodes {
            match &node.content {
                ...
                NodeContent::GapFill { gap_description, .. } => {
                    println!("    • Node {}: Gap - {} (conf: {:.2})", 
                             id, gap_description, node.confidence);
                }
                NodeContent::Decision { condition, .. } => {
                    println!("    • Node {}: Decision - {} (conf: {:.2})", 
    ...
    match MemoryGraph::load(memory_file) {
        Ok(loaded_memory) => {
            ...
            println!("    - Compiled modules: {}", loaded_memory.compiled_modules.len());
            ...
            for (id, fragment) in &loaded_memory.fragments {
                ...
                match &fragment.content {
                    FragmentContent::CausalRule { condition, outcome, .. } => {
                        println!("    • {}: {} → {} (conf: {:.2}, reinforced: {}x)", 
                                 id, condition, outcome, fragment.confidence, 
    ...
    if !memory.fragments.is_empty() {
        ...
        println!("  - Reinforcement count: {}", fragment_before.reinforcement_count);
        ...
        println!("\nAfter reinforcement:");
        println!("  - Confidence: {:.2} (improved by {:.2})", 
                 fragment_after.confidence,
                 fragment_after.confidence - fragment_before.confidence);
        println!("  - Reinforcement count: {}", fragment_after.reinforcement_count);
    }
