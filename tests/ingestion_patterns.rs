...
fn test_personal_pattern_extraction() {
    ...
    let has_name = matches.iter().any(|m| {
        m.content.get("fact_type") == Some(&"name".to_string())
    });
    assert!(has_name, "Should extract name");
    
    
    let has_preference = matches.iter().any(|m| {
        m.content.contains_key("preference")
    });
...
fn test_temporal_pattern_extraction() {
    ...
    let has_temporal = matches.iter().any(|m| {
        matches!(m.atom_type, AtomType::Time)
    });
...
fn test_spatial_pattern_extraction() {
    ...
    let has_location = matches.iter().any(|m| {
        matches!(m.atom_type, AtomType::Location)
    });
...
fn test_quantitative_pattern_extraction() {
    ...
    assert!(!matches.is_empty(), "Should extract quantitative information");
    
    
    let has_quantity = matches.iter().any(|m| {
        matches!(m.atom_type, AtomType::Quantity)
    });
...
fn test_hierarchical_pattern_extraction() {
    ...
    assert!(!matches.is_empty(), "Should extract hierarchical relationships");
    
    
    let has_hierarchical = matches.iter().any(|m| {
        m.content.contains_key("hierarchical_marker")
    });
...
fn test_social_pattern_extraction() {
    ...
    let has_social = matches.iter().any(|m| {
        m.content.contains_key("social_marker")
    });
...
fn test_ownership_pattern_extraction() {
    ...
    assert!(!matches.is_empty(), "Should extract ownership relationships");
    
    
    let has_ownership = matches.iter().any(|m| {
...
fn test_state_pattern_extraction() {
    ...
    let has_state = matches.iter().any(|m| {
        m.content.contains_key("state_marker")
    });
...
fn test_technical_pattern_extraction() {
    ...
    let has_technical = matches.iter().any(|m| {
        matches!(m.atom_type, AtomType::Entity | AtomType::Action)
    });
...
fn test_comprehensive_extraction() {
    ...
    assert!(!matches.is_empty(), "Should extract multiple types of information");
    
    
    let has_personal = matches.iter().any(|m| matches!(m.atom_type, AtomType::Person));
    let has_temporal = matches.iter().any(|m| matches!(m.atom_type, AtomType::Time));
    let has_spatial = matches.iter().any(|m| matches!(m.atom_type, AtomType::Location));
...
fn test_fragment_extraction_personal_fact() {
    ...
    let has_personal_fact = fragments.iter().any(|f| {
        matches!(f.fragment_type, FragmentType::PersonalFact)
    });
...
fn test_fragment_extraction_temporal_event() {
    ...
    let has_temporal_event = fragments.iter().any(|f| {
        matches!(f.fragment_type, FragmentType::TemporalEvent)
    });
...
fn test_fragment_extraction_social_relation() {
    ...
    let has_social_relation = fragments.iter().any(|f| {
        matches!(f.fragment_type, FragmentType::SocialRelation)
    });
...
fn test_enhanced_ingestion() {
    ...
    assert!(!event.relationships.is_empty(), "Should extract relationships");
...
fn test_extraction_stats() {
    ...
    assert_eq!(stats.pattern_match_counts.get("name_intro_my_name_is"), Some(&1));
