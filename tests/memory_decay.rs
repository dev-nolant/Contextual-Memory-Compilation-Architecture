...
fn test_reinforced_fragments_resist_decay() {
    ...
    if let (Some(decayed1), Some(decayed2)) = (
        memory.fragments.get(&frag1.id),
        memory.fragments.get(&frag2.id),
    ) {
...
fn test_decay_rate_calculation() {
    ...
    if let Some(decayed) = memory.fragments.get(&fragment.id) {
        ...
        assert!(
            (decayed.salience - expected).abs() < 0.2,
            "Salience decayed to {}, expected ~{}",
            decayed.salience,
            expected
        );
    } else {
        ...
        assert!(
            expected_after_decay < 0.01 || expected_after_decay >= 0.01,
            "Fragment removed but expected salience {} should have been preserved",
