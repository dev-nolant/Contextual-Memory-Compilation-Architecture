...
impl QueryExpander {
    pub fn expand_query(
        ...
        let related_keywords =
            Self::discover_related_keywords(&matching_fragments, memory, max_expansions);
        for keyword in related_keywords {
            expanded.insert(keyword);
        }

        let co_activation_keywords =
            Self::discover_co_activation_keywords(&matching_fragments, memory);
    ...
    fn extract_key_terms(query: &str) -> Vec<String> {
        let minimal_stop_words: HashSet<&str> = ["a", "an", "the", "is", "are", "was", "were"]
            .iter()
            .cloned()
            .collect();
    ...
    fn extract_keywords_from_fragments(
        fragment_ids: &[Uuid],
        memory: &MemoryGraph,
    ) -> HashSet<String> {
    ...
    fn discover_related_keywords(
        ...
        let mut to_explore: Vec<(Uuid, usize)> = fragment_ids.iter().map(|&id| (id, 0)).collect();
        ...
        while let Some((fragment_id, depth)) = to_explore.pop() {
            ...
            let related_fragments: Vec<Uuid> = memory
                .edges
                .iter()
    ...
    fn discover_co_activation_keywords(
        ...
        for pattern in &memory.co_activation_patterns {
            let has_match = fragment_ids
                .iter()
                .any(|id| pattern.fragment_ids.contains(id));
...
mod tests {
    ...
    fn create_test_memory_with_compound_key(
        base_term: &str,
        compound_key: &str,
        value: &str,
    ) -> MemoryGraph {
    ...
    fn test_expand_query_dynamic() {
        ...
        assert!(
            expanded.contains("entity"),
            "Should contain base term from query"
        );

        assert!(
            expanded.contains("entity_name"),
            "Should discover compound key from fragment content. Expanded: {:?}",
            expanded
        );
    }
    ...
    fn test_expand_query_discovers_related_keywords() {
        ...
        assert!(
            expanded.contains("base_identifier")
                || expanded
                    .iter()
                    .any(|k| k.contains("base") && k.contains("identifier")),
            "Should discover compound key. Expanded: {:?}",
            expanded
        );
    }
}
