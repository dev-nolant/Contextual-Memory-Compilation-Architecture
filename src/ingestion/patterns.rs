...
impl PatternLearner {
    ...
    pub fn extract_atoms_structural(text: &str) -> Vec<PatternMatch> {
        ...
        for i in 0..words.len().saturating_sub(1) {
            ...
            let is_likely_verb = next_word.len() >= 3
                && next_word.len() <= 6
                && (next_word == "like"
                    || next_word == "prefer"
                    || next_word == "enjoy"
        ...
        if !is_question {
            for i in 0..words.len().saturating_sub(2) {
                ...
                if is_assignment {
                    ...
                    if key_start > 0 {
                        let prev_word =
                            words[key_start - 1].trim_matches(|c: char| !c.is_alphanumeric());
                    ...
                    let key_parts: Vec<String> = words
                        .iter()
                        .skip(key_start)
                        .take(i + 1 - key_start)
                        .map(|w| {
                    ...
                    let value_parts: Vec<String> = words
                        .iter()
                        .skip(i + 2)
                        .take(5)
                        .map(|w| {
...
pub mod ownership {
    ...
    pub fn extract_ownership_atoms(text: &str) -> Vec<PatternMatch> {
        extract_all_patterns(text)
            .into_iter()
            .filter(|m| m.content.contains_key("ownership_marker"))
            .collect()
    }
}
