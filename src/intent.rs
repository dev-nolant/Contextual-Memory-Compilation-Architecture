...
impl IntentClassifier {
    ...
    pub fn classify_intent_with_text(
        semantic_event: &SemanticEvent,
        original_text: &str,
    ) -> Intent {
        let atom_types: Vec<String> = semantic_event
            .atoms
            .iter()
        ...
        let relationship_patterns: Vec<String> = semantic_event
            .relationships
            .iter()
    ...
    fn derive_intent_pattern(
        ...
        let question_starters = [
            "what", "who", "where", "when", "why", "how", "which", "whose", "whats", "what's",
            "whos", "who's", "wheres", "where's", "whens", "when's", "whys", "why's", "hows",
            "how's", "is", "are", "was", "were", "do", "does", "did", "can", "could", "will",
        ...
        let has_question_starter = question_starters.iter().any(|starter| {
            text_lower.starts_with(starter) || text_lower.starts_with(&format!("{} ", starter))
        });

        let has_incomplete = atoms
            .iter()
            .any(|a| a.content.values().any(|v| v.is_empty() || v == "unknown"));

        let has_question = atoms.iter().any(|a| a.content.contains_key("question"))
            || original_text.trim().ends_with('?');

        let has_interrogative_action = Self::detect_interrogative_patterns(atoms, relationships);

        let has_person = atoms
            .iter()
            .any(|a| matches!(a.atom_type, AtomType::Person));
        let has_location = atoms
            .iter()
            .any(|a| matches!(a.atom_type, AtomType::Location));
        let has_action = atoms
            .iter()
            .any(|a| matches!(a.atom_type, AtomType::Action));

        let has_requires = relationships
            .iter()
            .any(|r| matches!(r.relation_type, RelationType::Requires));
        let has_related = relationships
            .iter()
            .any(|r| matches!(r.relation_type, RelationType::RelatedTo));

        let word_count = original_text.trim().split_whitespace().count();
        let has_only_person_atoms = !atoms.is_empty()
            && atoms
                .iter()
                .all(|a| matches!(a.atom_type, AtomType::Person));
        ...
        let has_only_structural_relationships = relationships.is_empty()
            || relationships.iter().all(|r| {
                let is_structural = matches!(
                    r.relation_type,
                    RelationType::RelatedTo | RelationType::Semantic | RelationType::SimilarTo
                );

                let both_person = r.from_atom < atoms.len()
                    && r.to_atom < atoms.len()
        ...
        let has_no_meaningful_content = !has_action
            && !has_location
            && atoms.iter().all(|a| {
                matches!(a.atom_type, AtomType::Person) || matches!(a.atom_type, AtomType::Concept)
            });
        ...
        let has_complete_values = atoms.iter().any(|a| {
            matches!(
                a.atom_type,
                AtomType::Object | AtomType::Entity | AtomType::Concept
                ...
                && relationships.iter().any(|rel| {
                    ...
                    if person_is_involved {
                        ...
                        if other_atom_idx < atoms.len() {
                            ...
                            return matches!(
                                other_atom.atom_type,
                                AtomType::Entity | AtomType::Object
        ...
        let is_preference_statement = has_action
            && has_complete_values
            && !has_question_starter
            && (relationships.iter().any(|rel| {
    ...
    fn detect_interrogative_patterns(
        ...
        let incomplete_atom_indices: std::collections::HashSet<usize> = atoms
            .iter()
            .enumerate()
            .filter(|(_, atom)| {
                atom.content
                    .values()
                    .any(|v| v.is_empty() || v == "unknown" || v == "?")
            })
            .map(|(i, _)| i)
            .collect();
        ...
        for (i, atom) in atoms.iter().enumerate() {
            if matches!(atom.atom_type, AtomType::Action) {
                ...
                let has_incomplete_content = atom
                    .content
                    .values()
                    .any(|v| v.is_empty() || v == "unknown" || v == "?");

                let only_has_generic_key =
                    atom.content.len() == 1 && atom.content.contains_key("key");
                ...
                if has_incomplete_content || only_has_generic_key {
                    let has_complete_targets = atoms.iter().any(|a| {
                        !matches!(a.atom_type, AtomType::Action)
                            && a.content
                                .values()
                                .any(|v| !v.is_empty() && v != "unknown" && v != "?" && v != "key")
                    });
        ...
        let has_reference_entities = atoms.iter().any(|a| {
            matches!(
                a.atom_type,
                AtomType::Entity | AtomType::Concept | AtomType::Property
            ) && (a.content.len() == 1 && a.content.contains_key("key"))
                || a.content
                    .values()
                    .all(|v| v.is_empty() || v == "unknown" || v == "key" || v.len() <= 2)
        });

        if has_reference_entities {
            let has_actions = atoms
                .iter()
                .any(|a| matches!(a.atom_type, AtomType::Action));
            if has_actions {
                return true;
    ...
    pub fn match_intent_to_memory(intent: &Intent, memory: &MemoryGraph) -> Vec<Uuid> {
