...
fn main() {
    ...
    let mut memory = match MemoryGraph::load(DEFAULT_MEMORY_FILE) {
        Ok(mem) => {
            println!("Loaded memory from {} ({} fragments, {} edges)", 
                DEFAULT_MEMORY_FILE, mem.fragments.len(), mem.edges.len());
            mem
        }
        Err(_) => {
    ...
    loop {
        ...
        match command {
            "ingest" => {
                ...
                println!("✓ Ingested: {} atoms, created {} fragments", event.atoms.len(), fragments.len());
                ...
                if !fragments.is_empty() {
                    println!("\n  Created Fragments:");
                    let mut fragment_type_counts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
                    for fragment in &fragments {
                ...
                if !event.relationships.is_empty() {
                    println!("\n  Relationships: {} connections", event.relationships.len());
                }
            }
            "compile" => {
                if parts.len() < 3 {
            ...
            "eeg" => {
                if let Some(ref eeg) = last_eeg {
                    ...
                    for (id, node) in &eeg.nodes {
                        println!("    {}: {:?} (conf: {:.2})", id, node.node_type, node.confidence);
                    }
                    println!("  Edges:");
            ...
            "load" => {
                ...
                match MemoryGraph::load(path) {
                    Ok(mem) => {
                        memory = mem;
                        println!("Memory loaded from {} ({} fragments, {} edges)", 
                            path, memory.fragments.len(), memory.edges.len());
                    }
            ...
            "help" => {
                ...
                println!("  save [path]       - Save memory to file (default: {})", DEFAULT_MEMORY_FILE);
                println!("  load [path]       - Load memory from file (default: {})", DEFAULT_MEMORY_FILE);
