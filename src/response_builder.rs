...
impl ResponseBuilder {
    ...
    pub fn extract_top_candidates(
        ...
        eprintln!("   Execution trace length: {}", execution_result.execution_trace.len());
        ...
        let query_fragments: Vec<usize> = memory_data.fragments
            .iter()
            .enumerate()
        ...
        let mut answer_fragments = Self::find_answer_fragments(
        ...
        if !execution_result.execution_trace.is_empty() {
            ...
            for (frag_id, confidence) in graph_traversal_answers {
                if !answer_fragments.contains(&frag_id) {
                    ...
                    eprintln!("   Added answer fragment from graph traversal: {} (confidence={:.2})", frag_id, confidence);
                }
            }
        }

        eprintln!("   Answer fragment IDs: {:?} ({} fragments)", 
        ...
        let scored_candidates: Vec<(String, f64)> = candidates.into_iter()
            .map(|(value, base_confidence)| {
                
                
                let structural_confidence = if execution_result.execution_trace.iter().any(|query_frag_id| {
                    memory.edges.iter().any(|((from_id, to_id), edge)| {
                        
                        answer_fragments.iter().any(|answer_frag_id| {
                            let connects_query_to_answer = (*from_id == *query_frag_id && *to_id == *answer_frag_id) ||
                                                           (*to_id == *query_frag_id && *from_id == *answer_frag_id);
            ...
            .filter(|(_, confidence)| {
                ...
                if !keep {
                    eprintln!("   Filtered out candidate (confidence {:.3} <= 0.3)", confidence);
                }
                keep
            })
            .collect();

        eprintln!("   Scored candidates after filtering: {}", scored_candidates.len());

        
        let mut top_candidates: Vec<(String, f64)> = scored_candidates.into_iter()
            .collect();
        ...
        
        {
            ...
            let query_references_fallback: HashSet<String> = memory_data.fragments
                .iter()
                .flat_map(|frag| {
            ...
            let query_keywords: HashSet<String> = memory_data.query
                .split_whitespace()
            ...
            for (_fragment_id, frag) in memory.fragments.iter() {
                match &frag.content {
                    FragmentContent::PersonalFact { fact_type, value, .. } => {
                        
                        let fact_type_lower = fact_type.to_lowercase();
                        
                        
                        if exclusion_terms.iter().any(|excl| fact_type_lower.contains(excl)) {
                            eprintln!("   Skipping PersonalFact (matches exclusion): fact_type={}", fact_type);
                            continue;
                        }
                        ...
                        if query_keywords.iter().any(|kw| fact_type_lower.contains(kw)) && 
                           !value.is_empty() && value != "unknown" {
                            ...
                            eprintln!("   Candidate PersonalFact: fact_type={}, value={}, score={:.3}", fact_type, value, score);
                    ...
                    FragmentContent::SemanticAtom { atom_type, content, .. } => {
                        ...
                        if let Some(name_value) = name_value_opt {
                            if !name_value.is_empty() && name_value != "unknown" {
                                ...
                                let matches_exclusion = exclusion_terms.iter().any(|excl| name_lower.contains(excl));
                                
                                
                                if matches_exclusion {
                                    eprintln!("   Skipping Person name (matches exclusion): name={}", name_value);
                            continue;
                                }
                                ...
                                if query_keywords.contains("name") || query_lower.contains("name") {
                                    ...
                                    if name_value.chars().all(|c| c.is_alphabetic()) && name_value.len() <= 20 {
                                        score += 0.1;
                                    }
                                    
                                    if matches!(atom_type, AtomType::Entity) && content.contains_key("ownership_marker") {
                                        score += 0.05;
                                    }
                                    
                                    let matches_query_ref = query_references_fallback.contains(name_value) || 
                                                           query_references_fallback.contains(&name_lower);
                                    
                                    if matches_query_ref {
                                        score -= 0.1; 
                                    }
                                    eprintln!("   Candidate Person name (from {:?}): name={}, score={:.3}", atom_type, name_value, score);
                                    all_candidates.push((name_value.clone(), score, "PersonName".to_string()));
                                }
                            }
                        }
                            ...
                            if let Some(value) = content.get("key") {
                                if !value.is_empty() && value != "unknown" {
                                    ...
                                    let is_query_entity = query_references_fallback.contains(value) || 
                                                         query_references_fallback.contains(&value.to_lowercase());
                                    ...
                                    let query_has_action = memory_data.fragments.iter().any(|frag| {
                                        ...
                                        (frag.atom_type == "Entity" && 
                                         frag.content.get("key").map(|k| k.len() > 2 && k.len() < 10).unwrap_or(false) &&
                                    ...
                                    if query_has_action {
                                        ...
                                        if is_query_entity && Self::is_meaningful_entity(value) {
                                            ...
                                            eprintln!("   Candidate Object: value={}, score={:.3}", value, score);
                                        }
                                        all_candidates.push((value.clone(), score, "Object".to_string()));
                                    }
                                }
                            }
                        }
                                ...
                                if !value.is_empty() && value != "unknown" {
                                    ...
                                    if query_has_action {
                                        ...
                                        if is_query_entity && Self::is_meaningful_entity(value) {
                                            ...
                                            eprintln!("   Candidate Action: value={}, score={:.3}", value, score);
                                        }
                                        all_candidates.push((value.clone(), score, "Action".to_string()));
                                    }
                                }
                            }
                        }
                        ...
                        if matches!(atom_type, AtomType::Entity) {
                            if let Some(value) = content.get("key") {
                                if !value.is_empty() && value != "unknown" && value != "name" {
                                    ...
                                    if exclusion_terms.iter().any(|excl| value.to_lowercase().contains(excl)) {
                                        continue;
                                    }
                                    ...
                                    if query_has_action && content.contains_key("ownership_marker") {
                                        ...
                                        if value.chars().all(|c| c.is_alphabetic()) && value.len() <= 20 {
                                            score += 0.1;
                                        }
                                        ...
                                        all_candidates.push((value.clone(), score, "EntityPreference".to_string()));
                                    }
                                }
                            }
                        }
                        ...
                        for (key, value) in content {
                            ...
                            if value.is_empty() || value == "unknown" || value == "key" ||
                               query_references_fallback.contains(value) || query_references_fallback.contains(&value_lower) {
                                continue;
                            }
                            
                            
                            if exclusion_terms.iter().any(|excl| key_lower.contains(excl)) {
                                eprintln!("   Skipping SemanticAtom (key matches exclusion): {}={}", key, value);
                                continue;
                            }
                            ...
                            if query_keywords.iter().any(|kw| key_lower.contains(kw)) {
                                ...
                                let looks_like_answer = value.contains('-') || value.contains('_') || 
                                                      value.len() > 5 || 
                                ...
                                if looks_like_answer {
                                    ...
                                    eprintln!("   Candidate SemanticAtom: {}={}, score={:.3}", key, value, score);
                                    all_candidates.push((value.clone(), score, "SemanticAtom".to_string()));
                                }
                            }
                        }
                    }
            ...
            if !all_candidates.is_empty() {
                eprintln!("   Found {} candidates, ranking statistically...", all_candidates.len());
                ...
                for (i, (value, score, source)) in all_candidates.iter().take(5).enumerate() {
                    eprintln!("     Rank {}: value='{}', score={:.3}, source={}", i + 1, value, score, source);
                }
                ...
                if !top_candidates.is_empty() {
                    eprintln!("   ✅ Returning top {} candidates from fallback search", top_candidates.len());
                    return top_candidates;
                }
            }
    ...
    fn is_reference_value(value: &str) -> bool {
        value.len() <= 15 &&
        value.chars().all(|c| c.is_lowercase() || c == '_' || c == '-') &&
        !value.contains(' ') &&
        value.chars().all(|c| c.is_alphabetic() || c == '_' || c == '-')
    }
    ...
    fn is_meaningful_entity(value: &str) -> bool {
        ...
        let common_words: &[&str] = &["like", "i", "do", "is", "are", "was", "were", "and", "or", "the", "a", "an", 
                                      "my", "your", "his", "her", "their", "our", "what", "who", "where", "when", 
                                      "why", "how", "which", "that", "this", "these", "those", "have", "has", "had"];
        ...
        for fragment_id in answer_fragment_ids {
            if let Some(frag) = memory.fragments.get(fragment_id) {
                ...
                FragmentContent::SemanticAtom { content, .. } => {
                    ...
                    FragmentContent::PersonalFact { value, fact_type, .. } => {
    ...
    fn find_answer_fragments(
        activated_fragment_ids: &[Uuid],
        memory: &MemoryGraph,
    ) -> Vec<Uuid> {
        let mut answer_fragments = HashSet::new();
        
        eprintln!("\n🔍 [DEBUG] Finding answer fragments from {} activated fragments", activated_fragment_ids.len());
        
        
        let all_personal_facts: Vec<_> = memory.fragments.iter()
            .filter(|(_, frag)| matches!(frag.fragment_type, FragmentType::PersonalFact))
            .collect();
        eprintln!("   Total PersonalFact fragments in memory: {}", all_personal_facts.len());
        for (id, frag) in &all_personal_facts {
            if let FragmentContent::PersonalFact { person, fact_type, value, .. } = &frag.content {
                let is_activated = activated_fragment_ids.contains(id);
                eprintln!("     PersonalFact {}: person={}, fact_type={}, value={}, activated={}", 
        ...
        for fragment_id in activated_fragment_ids {
            ...
            for ((from_id, to_id), edge) in &memory.edges {
                ...
                if let Some(connected_id) = connected_id {
                    ...
                    let is_answer_edge = matches!(edge.edge_type, 
                        EdgeType::Semantic | 
                        EdgeType::Contextual
                    );
                    ...
                    if is_answer_edge && edge.strength > 0.3 {
                        if let Some(frag) = memory.fragments.get(&connected_id) {
                            ...
                            let is_answer_type = matches!(frag.fragment_type,
        ...
        for fragment_id in activated_fragment_ids {
            if let Some(frag) = memory.fragments.get(fragment_id) {
                ...
                if Self::is_answer_fragment(frag) {
                    eprintln!("     Found answer fragment in activated set: {} (has complete values)", fragment_id);
                }
            }
        }
        ...
        for fragment_id in activated_fragment_ids {
            if let Some(frag) = memory.fragments.get(fragment_id) {
                ...
                match &frag.fragment_type {
                    ...
                    FragmentType::OwnershipRelation => {
                        if let FragmentContent::OwnershipRelation { owner, owned, .. } = &frag.content {
    ...
    fn find_answers_via_graph_traversal(
        ...
        let query_has_action = memory_data.fragments.iter().any(|frag| frag.atom_type == "Action");
        let query_has_person = memory_data.fragments.iter().any(|frag| frag.atom_type == "Person");
                            ...
                            let matches_query_structure = match &frag.content {
                                FragmentContent::SemanticAtom { atom_type, content, .. } => {
                                    ...
                                    if query_has_action {
                                        ...
                                        content.get("key").map(|k| !k.is_empty() && k != "unknown").unwrap_or(false)
                                    } else {
    ...
    fn extract_values_from_fragments(
        ...
        eprintln!("\n🔍 [DEBUG] Extracting values from {} answer fragments", answer_fragment_ids.len());
        ...
        eprintln!("   Query references to exclude: {:?}", &query_references.iter().take(15).collect::<Vec<_>>());

        for fragment_id in answer_fragment_ids {
            if let Some(frag) = memory.fragments.get(fragment_id) {
                match &frag.content {
                    FragmentContent::PersonalFact { value, fact_type, person, .. } => {
                        eprintln!("   Checking PersonalFact: person={}, fact_type={}, value={}", person, fact_type, value);
                        if !value.is_empty() && value != "unknown" {
                            
                            if query_references.contains(value) || query_references.contains(&value.to_lowercase()) {
                    ...
                    FragmentContent::OwnershipRelation { owned, .. } => {
                        if !owned.is_empty() && owned != "unknown" {
                            ...
                            if query_references.contains(owned) || query_references.contains(&owned.to_lowercase()) {
                                continue;
                            }
                    ...
                    FragmentContent::SemanticAtom { atom_type, content, .. } => {
                        ...
                        if matches!(atom_type, AtomType::Object) {
                            ...
                            if let Some(value) = content.get("key") {
                                if !value.is_empty() && value != "unknown" {
                                    ...
                                    let query_action_key = memory_data.fragments.iter()
                                        .find(|frag| frag.atom_type == "Action")
                                        .and_then(|frag| frag.content.get("key"));
                                    
                                    let value_in_query = memory_data.query.to_lowercase().contains(&value.to_lowercase());
                                    
                                    
                                    let is_preference_object = if let Some(action_key) = query_action_key {
                                        memory.edges.iter().any(|((from_id, to_id), edge)| {
                                            let connected_to_this = *from_id == *fragment_id || *to_id == *fragment_id;
                                            if connected_to_this {
                                                let other_id = if *from_id == *fragment_id { *to_id } else { *from_id };
                                                if let Some(other_frag) = memory.fragments.get(&other_id) {
                                                    if let FragmentContent::SemanticAtom { atom_type: other_type, content: other_content, .. } = &other_frag.content {
                                                        return matches!(other_type, AtomType::Action) &&
                                                               other_content.get("key").map(|k| k == action_key).unwrap_or(false);
                                                    }
                                                }
                                            }
                                            false
                        ...
                        if matches!(atom_type, AtomType::Entity) {
                            if let Some(value) = content.get("key") {
                                ...
                                let is_query_reference = query_references.contains(value) || query_references.contains(&value.to_lowercase());
                                
                                
                                
                                
                                let connected_to_answer = answer_fragment_ids.iter().any(|other_frag_id| {
                                    if *other_frag_id == *fragment_id {
                                        return false;
                                    }
                                    memory.edges.iter().any(|((from_id, to_id), edge)| {
                                        ((*from_id == *fragment_id && *to_id == *other_frag_id) ||
                                         (*to_id == *fragment_id && *from_id == *other_frag_id)) &&
                                        edge.strength > 0.3
                                    })
                                });
                                ...
                                let query_asks_for_name_check = memory_data.fragments.iter().any(|frag| {
                                    frag.content.get("key").map(|k| k == "name").unwrap_or(false)
                                });
                                
                                let query_action_key = if !query_asks_for_name_check {
                                    memory_data.fragments.iter()
                                        .find(|frag| frag.atom_type == "Action")
                                        .and_then(|frag| frag.content.get("key"))
                                } else {
                                    None  
                                ...
                                let query_asks_for_name = memory_data.fragments.iter().any(|frag| {
                                    frag.content.get("key").map(|k| k == "name").unwrap_or(false)
                                });
                                
                                let is_name_value = if query_asks_for_name {
                                    
                                    let connected_via_edge = memory.edges.iter().any(|((from_id, to_id), edge)| {
                                        let connected_to_this = *from_id == *fragment_id || *to_id == *fragment_id;
                                        if connected_to_this && edge.strength > 0.3 {
                                            let other_id = if *from_id == *fragment_id { *to_id } else { *from_id };
                                            if let Some(other_frag) = memory.fragments.get(&other_id) {
                                                if let FragmentContent::SemanticAtom { content: other_content, .. } = &other_frag.content {
                                                    return other_content.get("key").map(|k| k == "name").unwrap_or(false);
                                                }
                                            }
                                        }
                                        false
                                    ...
                                    let connected_via_activation = answer_fragment_ids.iter().any(|other_frag_id| {
                                        if *other_frag_id == *fragment_id {
                                            return false;
                                        }
                                        if let Some(other_frag) = memory.fragments.get(other_frag_id) {
                                            if let FragmentContent::SemanticAtom { content: other_content, .. } = &other_frag.content {
                                                return other_content.get("key").map(|k| k == "name").unwrap_or(false);
                                            }
                                        }
                                ...
                                let is_general_preference = query_has_action && content.contains_key("ownership_marker");
                        ...
                        for (key, value) in content {
                            ...
                            let connected_to_answer_fragments = answer_fragment_ids.iter().any(|other_frag_id| {
                                if *other_frag_id == *fragment_id {
                                    return false;
                                }
                                memory.edges.iter().any(|((from_id, to_id), edge)| {
                            ...
                            let key_matches_query = query_text_lower.contains(&key.to_lowercase()) ||
                                                   query_text_lower.split_whitespace().any(|w| key.to_lowercase().contains(w));
    ...
    fn calculate_value_confidence(
        ...
        if query_keys.contains(key) {
            confidence += 0.3;
        } else if query_values.iter().any(|qv| qv.contains(key) || key.contains(qv)) {
    ...
    pub fn format_candidates_for_llm(
        ...
        for (i, (value, confidence)) in candidates.iter().enumerate() {
            if is_preference {
                candidate_list.push(format!("  {}. {} (confidence: {:.2})", i + 1, value, confidence));
            } else {
                candidate_list.push(format!("  {}. {} (confidence: {:.2})", i + 1, value, confidence));
            }
        }
    ...
    pub fn format_answer(
        ...
        let format_type = Self::detect_format_type_from_structure(
            memory_data,
            memory,
            execution_result,
        );
        ...
        match format_type {
            FormatType::Possessive { entity_key } => {
                ...
                let is_preference_query = Self::is_preference_query(answer, memory_data, memory, execution_result);
    ...
    fn is_preference_query(
        ...
        let query_has_preference_action = memory_data.fragments.iter().any(|frag| {
            frag.atom_type == "Action" || 
            (frag.atom_type == "Entity" && frag.content.get("key").map(|k| {
                let k_lower = k.to_lowercase();
                
                k_lower.len() >= 3 && k_lower.len() <= 6 && 
                query_lower.contains(&k_lower)
            }).unwrap_or(false))
        });
        
        
        let answer_from_preference_fragment = execution_result.execution_trace.iter().any(|frag_id| {
            if let Some(frag) = memory.fragments.get(frag_id) {
                match &frag.content {
                    FragmentContent::SemanticAtom { atom_type, content, .. } => {
                        
                        (matches!(atom_type, AtomType::Object) || matches!(atom_type, AtomType::Entity)) &&
        ...
        let memory_has_preference_pattern = memory_data.fragments.iter().any(|frag| {
            ...
            frag.content.contains_key("ownership_marker") &&
            frag.content.get("key").map(|k| k == answer || k.to_lowercase() == answer.to_lowercase()).unwrap_or(false)
        });
        
        query_has_preference_action && (answer_from_preference_fragment || memory_has_preference_pattern)
    }
                ...
                if let Some(frag) = memory.fragments.get(fragment_id) {
                    if let FragmentContent::SemanticAtom { content, .. } = &frag.content {
                        ...
                        for key in content.keys() {
                            if key != "key" && key != "ownership_marker" && !Self::is_reference_value(key) && key.len() > 3 {
                                let normalized = key.replace("_", " ").replace("-", " ");
                                entity_key = normalized;
                                break;
                            }
...
impl ResponseBuilder {
    ...
    pub fn build_response(
        ...
        if let Some((answer, confidence)) = Self::extract_answer(memory_data, memory, execution_result) {
            if confidence > 0.5 {
