...
"#, user_query, candidates_text);
    ...
    fn format_response(&self, result: &ExecutionResult, context: &ContextVector) -> Result<String, LLMError> {
...
impl MemoryData {
    ...
    pub fn from_execution_and_memory(
        ...
        let fragments: Vec<FragmentData> = execution_result.execution_trace
            .iter()
    ...
    pub fn to_compact_string(&self) -> String {
        ...
        let frags: Vec<String> = self.fragments.iter().map(|f| {
            let content: Vec<String> = f.content.iter()
                .filter(|(k, v)| {
...
impl CMCAgent {
    ...
    pub fn process(&mut self, user_input: &str) -> Result<String, LLMError> {
        ...
        let semantic_event = match self.llm.extract_semantics(user_input) {
            ...
            Err(e) => {
                if self.debug {
                    eprintln!("   ⚠️  LLM extraction failed: {:?}, falling back to structural extraction", e);
                }
        ...
        if self.debug {
            ...
            for (i, atom) in semantic_event.atoms.iter().enumerate() {
                eprintln!("     Atom {}: type={:?}, content={:?}", i, atom.atom_type, atom.content);
            }
        ...
        let intent = crate::intent::IntentClassifier::classify_intent_with_text(&semantic_event, user_input);
            ...
            for (i, frag) in fragments.iter().enumerate() {
                match &frag.content {
                    FragmentContent::PersonalFact { person, fact_type, value, .. } => {
                        eprintln!("     Fragment {}: PersonalFact - person={}, fact_type={}, value={}", 
                            i, person, fact_type, value);
                    }
                    FragmentContent::OwnershipRelation { owner, owned, .. } => {
                        eprintln!("     Fragment {}: OwnershipRelation - owner={}, owned={}", 
                            i, owner, owned);
                    }
                    FragmentContent::SemanticAtom { atom_type, content, .. } => {
                        eprintln!("     Fragment {}: SemanticAtom - type={:?}, content={:?}", 
                            i, atom_type, content);
                    }
                    _ => {
        ...
        if !is_query && !is_greeting {
            ...
            let edges = crate::distillation::create_edges_from_relationships(&semantic_event, &fragments);
            ...
            if self.debug {
                eprintln!("\n🔍 [DEBUG] Storing {} fragments in memory (statement)", fragments.len());
            }
            for fragment in &fragments {
                ...
                if self.debug {
                    match &fragment.content {
                        FragmentContent::PersonalFact { person, fact_type, value, .. } => {
                            eprintln!("   Storing PersonalFact: person={}, fact_type={}, value={}", 
                                person, fact_type, value);
                        }
                        FragmentContent::OwnershipRelation { owner, owned, .. } => {
                            eprintln!("   Storing OwnershipRelation: owner={}, owned={}", owner, owned);
                        }
                        FragmentContent::SemanticAtom { atom_type, content, .. } => {
                ...
                self.memory.insert_fragment(fragment.clone(), fragment_edges);
            }
        } else {
            if self.debug {
                if is_greeting {
        ...
        if is_query {
            ...
            let intent_matches = crate::intent::IntentClassifier::match_intent_to_memory(
                &intent,
                &self.memory
            );
        ...
        if self.debug {
            ...
            if !execution_result.execution_trace.is_empty() {
                eprintln!("   Activated fragments: {:?}", execution_result.execution_trace);
            }
        }
        
            
            let memory_data = MemoryData::from_execution_and_memory(
                &execution_result,
                &self.memory,
                user_input
            );
            ...
            if self.debug {
                ...
                for (i, frag) in memory_data.fragments.iter().enumerate() {
                    eprintln!("     Fragment {}: type={}, content={:?}", i, frag.atom_type, frag.content);
                }
            ...
            let response = if execution_result.confidence > 0.3 && !memory_data.fragments.is_empty() {
                ...
                if !candidates.is_empty() {
                    ...
                    self.llm.format_response_from_candidates(user_input, &candidates, &memory_data)
                        .unwrap_or_else(|_| {
                    ...
                    self.llm.format_response_from_memory(user_input, &memory_data)
            ...
            if !execution_result.execution_trace.is_empty() {
                ...
                self.memory.record_co_activation(&execution_result.execution_trace);
            }
            
            
            for signal in &execution_result.reinforcement_signals {
                self.memory.reinforce_fragment(
                    signal.fragment_id,
    ...
    fn add_query_keywords(&self, user_input: &str, context: &mut ContextVector) {
        ...
        let expanded_keywords = crate::query_expansion::QueryExpander::expand_query(
            user_input,
            &self.memory,
            2, 
        );
        ...
        let keywords: HashSet<String> = user_input.to_lowercase()
            .split_whitespace()
        ...
        let phrase = user_input.to_lowercase()
            .split_whitespace()
            .filter(|w| w.len() >= 2)
            .collect::<Vec<_>>()
            .join("");
...
pub mod openai_provider {
    ...
    impl OpenAIProvider {
        ...
        fn call_openai(&self, prompt: &str, system_prompt: Option<&str>, require_json: bool) -> Result<String, LLMError> {
            let messages = vec![
            ...
            if !response.status().is_success() {
                ...
                return Err(LLMError::ProviderError(format!("OpenAI API error {}: {}", status, text)));
            }
...
"#, text);
            ...
            let json_value: serde_json::Value = serde_json::from_str(cleaned_json)
                .map_err(|e| LLMError::ParseError(format!("Failed to parse JSON: {} (raw: {})", e, json_str)))?;
            
            
            let event_type_str = json_value["event_type"].as_str()
            ...
            let relationships: Vec<Relationship> = json_value["relationships"]
                ...
                .map(|rel_json| {
                    ...
                    let relation_type_str = rel_json["relation_type"].as_str().unwrap_or("Semantic");
                    ...
                    Relationship { from_atom, to_atom, relation_type, strength }
                })
                .collect();
        ...
        fn format_response_from_memory(&self, user_query: &str, memory_data: &MemoryData) -> Result<String, LLMError> {
...
"#, 
        ...
        fn format_response(&self, result: &ExecutionResult, _context: &ContextVector) -> Result<String, LLMError> {
...
pub mod anthropic_provider {
    ...
    impl AnthropicProvider {
        ...
        fn call_anthropic(&self, prompt: &str, system_prompt: Option<&str>, _require_json: bool) -> Result<String, LLMError> {
            ...
            if !response.status().is_success() {
                let status = response.status();
                let text = response.text().unwrap_or_default();
                return Err(LLMError::ProviderError(format!("Anthropic API error {}: {}", status, text)));
            }
...
pub mod local_provider {
    ...
    impl LLMProvider for LocalLLMProvider {
        ...
        fn format_response(&self, result: &ExecutionResult, _context: &ContextVector) -> Result<String, LLMError> {
            Ok(format!("Execution completed: {} (confidence: {:.2})", 
