...
pub fn extract_pattern(
    ...
    match candidate.pattern_type.as_str() {
        ...
        "BranchPattern" => {
            ...
            if let Some(branch_pattern) = pattern_report.stable_branches.iter().find(|b| {
                (b.branch_ratio - candidate.reward_correlation).abs() < 0.1
            }) {
                Some(extract_branch_pattern(branch_pattern, eegs, execution_traces))
            } else {
                None
            }
        }
        "SubgraphPattern" => {
            ...
            if let Some(subgraph_pattern) = pattern_report.invariant_subgraphs.iter().find(|s| {
                s.occurrence_count == candidate.repetition_count
            }) {
                Some(extract_subgraph_pattern(subgraph_pattern, eegs, execution_traces))
            } else {
                None
            }
        }
...
fn extract_path_pattern(
    ...
    if let Some((eeg, _)) = instances.first() {
        ...
        for i in 0..canonical_path.len().saturating_sub(1) {
            ...
            if let Some(edge) = eeg.edges.iter().find(|e| e.from_node == from && e.to_node == to) {
    ...
    let structure = PatternStructure {
        ...
        edges: canonical_path.windows(2)
            .map(|w| (w[0], w[1]))
            .collect(),
        node_types,
        edge_types,
...
fn infer_activation_condition(contexts: &[ContextVector]) -> ContextPattern {
    ...
    let avg_confidence: f64 = contexts.iter().map(|c| c.confidence_threshold).sum::<f64>() / contexts.len() as f64;
...
pub fn compile_to_fsm(extracted_pattern: &ExtractedPattern) -> CompiledModule {
    ...
    for (from_node, to_node) in &extracted_pattern.structure.edges {
        let from_idx = extracted_pattern.structure.nodes.iter().position(|&n| n == *from_node);
        let to_idx = extracted_pattern.structure.nodes.iter().position(|&n| n == *to_node);
...
fn module_matches_context(module: &CompiledModule, context: &ContextVector) -> bool {
    ...
    let matches_goal = module.activation_condition.goal_patterns.iter().any(|pattern| {
        goal_lower.contains(pattern)
    }) || module.activation_condition.goal_patterns.is_empty();
    
    
    let matches_domain = module.activation_condition.domain_hints.contains(&context.domain_hint.domain)
        || module.activation_condition.domain_hints.is_empty();
    
    
    let matches_confidence = context.confidence_threshold >= module.activation_condition.confidence_threshold;
    
    matches_goal && matches_domain && matches_confidence
}

pub fn execute_compiled_module(
